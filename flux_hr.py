...
# Create summary flux table


base_output = outputDir + "/commonFiles/results/flux-hardness/flux_hardness_table"
counter = 1
while Path(f"{base_output}_{counter}.txt").exists():
    counter += 1
output_file = f"{base_output}_{counter}.txt"
...
for path, obsid, expo in searchedObservations:
    ...
    for fluxModel in fluxes_to_be_calculated:
        ...
        # Collect into dict for summary table
        flux_dict[fluxModel.lower()] = flux
    ...
    # Soft band: 2-6 keV, Hard band: 6-10 keV
    ...
    try:
        ...
        soft_rate, soft_err = get_rate(2.0, 6.0)
        hard_rate, hard_err = get_rate(6.0, 10.0)
        hardness_ratio = hard_rate / soft_rate if soft_rate != 0 else 0.0

        print("\nCount Rates:")
        print(f"Soft (2-6 keV): {soft_rate:.5f} ct/s")
        print(f"Hard (6-10 keV): {hard_rate:.5f} ct/s")
    ...
    # Write one summary line per observation
    
    with open(output_file, "a") as f:
        
        if not header_written:
            header = "Obsid "
            for name in flux_dict.keys():
                header += f"{name}_flux {name}_flux_errlow {name}_flux_errhigh "
            header += "soft_rate soft_rate_err hard_rate hard_rate_err hardness_ratio\n"
            f.write(header)
            header_written = True
        ...
        for name in flux_dict.keys():
            vals = flux_dict[name]
            line += f"{vals[0]} {vals[1]} {vals[2]} "
        line += f"{soft_rate} {soft_err} {hard_rate} {hard_err} {hardness_ratio}\n"
        f.write(line)
